...
interface SubscriptionBadgeProps {
  ...
  status?: 'active' | 'activation_pending' | 'expired';
}

export function SubscriptionBadge({ planName, expiryDate, userRole, status = 'active' }: SubscriptionBadgeProps) {
  ...
  return (
    <div className={`flex items-center space-x-2 px-3 py-2 rounded-lg border text-sm font-medium ${getBadgeColor()}`}>
      ...
      <div>
        ...
        {planName === 'Trial' && (
          <div className="text-xs opacity-75">
            {status === 'activation_pending' ? 'Activation Pending' : 'Trial Period'}
          </div>
        )}
      </div>
    </div>
